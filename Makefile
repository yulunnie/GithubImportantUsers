...
# -----------------------------------------------------------
spark.root=/usr/local/Cellar/apache-spark/3.0.1
hadoop.root=/usr/local/Cellar/hadoop/3.3.0
app.name=UserRank
jar.name=UserRank-1.0.jar
maven.jar.name=UserRank-1.0.jar
job.name=UserRank
local.master=local[*]
local.input=input
local.output=output
# Pseudo-Cluster Execution
hdfs.user.name=yulunnie
hdfs.input=input
hdfs.output=output
# AWS EMR Execution
aws.emr.release=emr-6.2.0
aws.bucket.name=yulun
aws.input=input
aws.output=output
aws.log.dir=log
aws.num.nodes=5
